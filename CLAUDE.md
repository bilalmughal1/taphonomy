...
### 0.1.1 Scope of the attribution rule

This rule prohibits *attribution of authorship*. It does not prohibit
documenting which tools the project permits.

Prohibited:

* commit trailers, pull request footers, or file headers crediting a tool
* "Generated by", "Created with", "Assisted by", or equivalent phrasing in
  any tracked content
* any statement implying that a tool authored, co-authored, or holds rights
  in a change

Permitted:

* the filename `CLAUDE.md` and references to it
* naming coding agents in policy documents that govern their use
* naming a tool in an ADR that records a decision about tooling

The test is whether the text credits a tool for work, or states a rule about
tools. Crediting is prohibited. Stating policy is not.

Where a passage is ambiguous, treat it as attribution and omit it.
